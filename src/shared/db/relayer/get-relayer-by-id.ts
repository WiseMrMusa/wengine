...
export const getRelayerById = async ({
    id
}: GetRelayerByIdParams) => {
    ...
    return {
        ...
        allowedContracts: relayer.allowedContracts 
         ? (JSON.parse(relayer.allowedContracts).map((contractAddress: string) => contractAddress.toLowerCase())) as string[]
         : null,
    };
}