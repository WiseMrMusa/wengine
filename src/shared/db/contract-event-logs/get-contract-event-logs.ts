...
export const getContractEventLogsByBlockTimestamp = async ({
    fromBlockTimestamp,
    toBlockTimestamp,
    contractAddresses,
    topics
...
export const getEventsLogsByCursor = async ({
    cursor,
    limit = 100,
    contractAddresses,
    topics,
    ...
    if (cursor) {
        ...
        const [createdAt, chainId, blockNumber, transactionIndex, logIndex] = parsedCursor;
                ...
                ? [
                    {
                        OR: [
                            ...
                            },
                            {
                                ...
                                blockNumber: { equals: cursorObj.blockNumber },
                                transactionIndex: { gt: cursorObj.transactionIndex },
                                logIndex: { gte: cursorObj.logIndex }
                            }
                        ]
                     }] : []),
                     ...(maxCreatedAt 
    ...
    const cursorString = `${lastLog.createdAt.getTime()}-${lastLog.chainId}-${lastLog.blockNumber}-${lastLog.transactionIndex}-${lastLog.logIndex}`;
